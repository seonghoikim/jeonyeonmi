  ...
  return createPortal(
    <div style={{ background: "#fff", color: "#000", minHeight: "100vh" }}>
      ...
      <div style={{ maxWidth: 720, margin: "0 auto", padding: "48px 32px 64px" }}>
        ...
        <p style={{ fontSize: 13, letterSpacing: "0.1em", color: "#000", marginTop: 4, textTransform: "uppercase" }}>{u.cvTitle}</p>

        {contacts.length > 0 && (
          <section style={{ marginTop: 32 }}>
            <h2 style={{ fontSize: 11, letterSpacing: "0.15em", textTransform: "uppercase", color: "#000", borderBottom: "1px solid #ccc", paddingBottom: 6, marginBottom: 10 }}>{u.cvContact}</h2>
            {contacts.map((item) => (
        ...
        {current.length > 0 && (
          <section style={{ marginTop: 28 }}>
            <h2 style={{ fontSize: 11, letterSpacing: "0.15em", textTransform: "uppercase", color: "#000", borderBottom: "1px solid #ccc", paddingBottom: 6, marginBottom: 10 }}>{u.cvCurrent}</h2>
            {current.map((ex) => (
        ...
        {history.length > 0 && (
          <section style={{ marginTop: 28 }}>
            <h2 style={{ fontSize: 11, letterSpacing: "0.15em", textTransform: "uppercase", color: "#000", borderBottom: "1px solid #ccc", paddingBottom: 6, marginBottom: 10 }}>{u.cvHistory}</h2>
            {history.map((ex) => (
              <p key={ex.id} style={{ fontSize: 13, margin: "4px 0", lineHeight: 1.5 }}>
        ...
        <p style={{ fontSize: 10, color: "#000", marginTop: 48 }}>{u.cvGenerated}: {new Date().toISOString().slice(0, 10)}</p>
      </div>
    </div>,
    portalEl
  );
