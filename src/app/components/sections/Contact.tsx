              ...
              <div key={item.id}
                ...
                {isEditingThis ? (
                  ...
                  <div className="flex items-center justify-between group/ci">
                    ...
                    {editMode && <div className="flex items-center gap-1 pr-3 sm:pr-4 shrink-0"><button onClick={() => toggleContactVisibility(item.id)} className={`p-1.5 transition-colors ${item.visible ? "text-muted-foreground hover:text-foreground" : "text-accent"}`} title={item.visible ? u.contactHide : u.contactShow} aria-label={item.visible ? u.contactHide : u.contactShow}>{item.visible ? <Eye size={13} /> : <EyeOff size={13} />}</button><button onClick={() => setEditingContactId(item.id)} aria-label={u.editLabel} className="p-1.5 text-muted-foreground hover:text-foreground transition-colors"><Edit3 size={13} /></button></div>}
                  </div>
                )}
              </div>
            );
